(function (root, factory) {
  ...
  if (typeof module === 'object' && module.exports) {
    ...
    const engine = require('./v16.js');
    module.exports = factory({
      GardenSimulation: engine.GardenSimulation,
  ...
  const engine = root && root.AnomalyGardenSim;
  if (!engine || !engine.GardenSimulation || typeof engine.hashSeed !== 'function') {
    throw new Error('Anomaly Garden state contract requires v16 engine first');
  }

  root.AnomalyGardenStateContract = factory({
    GardenSimulation: engine.GardenSimulation,
    hashSeed: engine.hashSeed,
    sha256Utf8: null
  });
})(typeof globalThis !== 'undefined' ? globalThis : this, function createStateContract(deps) {
  'use strict';

  const GardenSimulation = deps.GardenSimulation;
  ...
  function sha256Canonical(value) {
    if (typeof deps.sha256Utf8 !== 'function') return null;
    return deps.sha256Utf8(canonicalJson(value));
  }
  ...
  function deserializeVerified(input) {
    if (!GardenSimulation || typeof GardenSimulation.deserialize !== 'function') {
      const error = new Error('Anomaly Garden state contract has no simulation restore adapter');
      error.code = 'AXM_STATE_RESTORE_UNAVAILABLE';
      throw error;
    }
    ...
    if (before.version === sim.version && before.stateSha256 !== null && after.stateSha256 !== null && before.stateSha256 !== after.stateSha256) {
